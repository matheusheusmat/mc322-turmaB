...
public class Validacao {
	...
	public static void listarTodos(ArrayList lista, String tipo) {
		int contador = 1;
		for (Object obj : lista) {
			System.out.printf("-- %s %d --\n%s\n\n", tipo, contador, obj.toString());
			contador++;
		}
	}
	
	public static boolean inicioAntesDoFim(Date dataInicio, Date dataFim) {
		int diaIni = dataInicio.getDate();
		int mesIni = dataInicio.getMonth() + 1;
		int anoIni = dataInicio.getYear() + 1900;
		LocalDate dataIniForm = LocalDate.of(anoIni, mesIni, diaIni);
		
		int diaFim = dataFim.getDate();
		int mesFim = dataFim.getMonth() + 1;
		int anoFim = dataFim.getYear() + 1900;
		LocalDate dataFimForm = LocalDate.of(anoFim, mesFim, diaFim);
		
		if (dataIniForm.isAfter(dataFimForm)) {
			return false;
		}
